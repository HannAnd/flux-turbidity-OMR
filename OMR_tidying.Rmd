...
dyeflux <- c("9B", "3C", "10A", "8A", "1A", "5C", "10B", "2C", "2B")
```
...
stim_resp_all <- all_agg %>%
                 filter(behavior %in% c("positive", "negative"))
stim_resp_lev <- all_lev %>%
                 filter(behavior %in% c("positive", "negative"))
  #other behaviors
oth_resp <- all_agg %>%
            filter(!behavior %in% c("positive", "negative"))
...
#unnecessary columns
  #positive and negative stimulus responses
stim_resp_all <- merge(stim_resp_all[,c(1:5,7:12)], all_comp, all = TRUE,
                       by = c("experiment", "shoalID", "treatment", "sex",
                              "OMR_ID", "water_clarity"))
stim_resp_lev <- merge(stim_resp_lev, all_comp, all = TRUE,
                       by = c("experiment", "shoalID", "treatment", "sex",
                              "OMR_ID", "water_clarity"))
  #other behaviors
...
#doesn't export) and removing any resultant incorrect NAs
  #positive and negative stimulus responses
stim_resp_all <- as.data.frame(stim_resp_all) %>%
...
stim_resp_lev <- as.data.frame(stim_resp_lev) %>%
                 complete(nesting(experiment, shoalID, treatment, sex, OMR_ID,
                                  water_clarity),
                                  stimulus_level, behavior,
                          fill = list(duration_s = NA, behav_count = 0)) %>%
