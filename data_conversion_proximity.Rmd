...
for (k in 1:(length(list_csv)/fish)) {  #across the whole file list
  ...
  for (a in 1:(maxframe+1)) {  #across one video of files
    ...
    #creating a dataframe to hold the coordinates for the four corners of the
    #filming area
    walls <- data.frame(matrix(nrow = 4, ncol = 2))
    colnames(walls) <- c("X", "Y")
    #enumerating the columns from 'corners' being placed into 'walls'
    XY = 3:4
    #adding the film area coordinates to the dataframe with the order of
    #bottom left corner, bottome right corner, top right corner, top left corner
    for (w in 1:4) {
      #####still need to check if this works!
      walls[w,] <- corners[corners$tankID == file_str[[1]][2] &
