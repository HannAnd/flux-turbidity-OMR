...
knitr::opts_knit$set(root.dir = "E://Turbidity Flux Experiment with OMR/calibrated_CSVs")
#loading packages
library(tidyverse)
library(data.table)
library(zoo)
...
corners <- read_csv("E://Turbidity Flux Experiment with OMR/calibrated_corner_coordinates.csv")
...
for (k in 1:(length(list_csv)/fish)) {  #across the whole file list
  ...
  #calculating the number of frames each grid is occupied by each fish during
  #the video
  ...
  fish3_coor <- shoal[[4]] %>%
                st_as_sf(coords = c("X", "Y"), remove = FALSE,
                          na.fail = FALSE)
  #detecting the number of frames each fish in each grid cell
  ...
  testpoly <- ggplot() +
              geom_sf(data = film_grid, fill = NA) +
              geom_sf(data = st_zm(film_poly), fill = NA, color = "red")
  ...
  for (a in 1:(maxframe+1)) {  #across one video of files
    ...
    #!!!!!!Need to deal with the fact that for specific videos there are only
    #three total fish- the current code would exclude any shoal calculations
    #for those videos!!!!!!#
...
write_csv(finalframe, "E://Turbidity Flux Experiment with OMR/6day_clay_proximity_data.csv")
```
