---
title: "Flu Turbidity Data Conversion for Proximity Tests, Batch Processing"
...
All below code is intended to be identical to the code from
"data_conversion_proximity.Rmd", but this code has been modified to allow for
running the datafiles in batches. Processing time for the original script
was estimated to take ~24 hours of active runtime on my computer, so this code
allows me to process the videos in smaller batches requiring less than 24 hours.
...
dyeclear <- c("tank1C", "tank5B", "tank3C", "tank8A", "tank9A", "tank4C",
              "tank6A", "tank6B", "tank2C", "tank2B")
dyestable <- c("tank1B", "tank9B", "tank8B", "tank3B", "tank9C", "tank7A",
               "tank3A", "tank6C", "tank1A", "tank10A")
dyeflux <- c("tank10C", "tank8C", "tank10B", "tank5C", "tank2A", "tank7C",
             "tank5A", "tank4B", "tank4A", "tank7B")
```
...
#the number of the first file to be processed
i = 1
#the number of videos to run in this batch of processing
batch <- 10
#the batch number, to be used when naming the exported file
batch_num <- 1

#probably should put a comment here
for (k in 1:(batch*fish)) {  #across the every video in the current batch
  ...
  #using the filename information to the datafile to be exported
...
#the file name for the exported file including the current batch number
filename <- "D://Turbidity Flux Experiment with OMR/6day_clay_proximity_data_"
filename <- paste0(filename, "batch", batch_num, ".csv")
write_csv(finalframe, filename)
#reminding the user to keep track of the batch and i values
remind <- paste0("WRITE THIS DOWN! This was batch ", batch_num,
                 ". The next batch will be batch ", (batch_num+1),
                 "! The next 'i' value will be ", i, "! WRITE THIS DOWN!")
print(remind)
```